// <copyright file="MaxIdStringRepository.cs" company="Lakstins Family, LLC">
...
namespace MaxFactry.Base.DataLayer
{
	...
	/// <summary>
    /// Provides static methods to manipulate storage of data using the MaxIdStringDataModel
	/// </summary>
    public abstract class MaxIdStringRepository : MaxStorageWriteRepository
	{
		/// <summary>
		/// Selects entity matching the unique identifier
		/// </summary>
        /// <param name="loData">Data used to determine repository provider.</param>
        /// <param name="lsId">Unique Identifier of the entity</param>
        ...
        public static MaxData SelectById(MaxData loData, string lsId, params string[] laDataNameList)
		{
            MaxIdStringDataModel loDataModel = loData.DataModel as MaxIdStringDataModel;
            if (null == loDataModel)
            {
                throw new MaxException("Error casting [" + loData.DataModel.GetType() + "] for DataModel");
            }

            MaxDataList loList = SelectAllByProperty(loData, loDataModel.Id, lsId, laDataNameList);
            ...
            {
                throw new MaxException("Multiple records match the  Id [" + lsId.ToString() + "]");
            }
